...
Graph *GraphCreateComplete(unsigned int numVertices, int isDigraph)
{
  ...
  unsigned int i = 0;
  for (; i < g->numVertices; ListMoveToNext(vertices), i++) // for each vertex
  {
    struct _Vertex *v = ListGetCurrentItem(vertices);
    List *edges = v->edgesList;
    for (unsigned int j = 0; j < g->numVertices; j++) // for each other vertex -> create an edge that connects both vertexes
    {
      if (i == j)
      {
        continue;
...
Graph *GraphCreateTranspose(const Graph *g)
{
  ...
  // COMPLETE THE CODE
  Graph *transpose = GraphCreate(g->numVertices, 1, g->isWeighted); // create graph with same number of vertices as original
  List *original_vertices = g->verticesList;
  List *transpose_vertices = transpose->verticesList; // no need to malloc because of ListCreate in GraphCreate!

  ListMoveToHead(original_vertices); // moves to the head of the vertexes list

  // first -> add all vertices to the transpose graph
  for (unsigned int i = 0; i < g->numVertices; ListMoveToNext(original_vertices), i++) // for each vertex of original
  {
    // fetch orginal info on vertex i
    struct _Vertex *original_vertex = ListGetCurrentItem(original_vertices);
    struct _Vertex *transpose_vertex = (struct _Vertex *)malloc(sizeof(struct _Vertex));
    if (!transpose_vertex)
    {
      abort();
    }

    transpose_vertex->id = original_vertex->id;                     // same id as original
    transpose_vertex->inDegree = original_vertex->outDegree;        // inDegree will be swapped with outDegree
    transpose_vertex->outDegree = original_vertex->inDegree;        // outDegree will be swapped with inDegree
    transpose_vertex->edgesList = ListCreate(graphEdgesComparator); // create list to hold edges that fall upon this vertex

    // add vertex to transpose_vertices list
    ListInsert(transpose_vertices, transpose_vertex);
  }

  ListMoveToHead(original_vertices);  // moves to the head of the vertexes list again
  ListMoveToHead(transpose_vertices); // moves to head of vertices list on transpose graph

  // second -> for each vertex go through each edge and find the adjacent vertex in transpose graph
  for (unsigned int i = 0; i < g->numVertices; ListMoveToNext(original_vertices), ListMoveToNext(transpose_vertices), i++)
  {
    struct _Vertex *original_vertex = ListGetCurrentItem(original_vertices);
    struct _Vertex *transpose_vertex = ListGetCurrentItem(transpose_vertices);

    List *original_edges = original_vertex->edgesList;

    ListMoveToHead(original_edges); // moves to head of edges list

    for (unsigned int j = 0; j < original_vertex->outDegree; ListMoveToNext(original_edges), j++) // for each edge that starts on this vertex
    {
      // fetch original info on edge j
      struct _Edge *original_edge = ListGetCurrentItem(original_edges);
      unsigned int target_vertex = original_edge->adjVertex;

      // add an edge that connects to i
      GraphAddEdge(transpose, target_vertex, transpose_vertex->id);
    }
  }
  return transpose;
}
